...
import com.hillyuen.utils.OwoUtil;
...
public class FrontCommentController {
    ...
    @ResponseBody
    public Result newComment(@ModelAttribute("comment") Comment comment,
                                 @ModelAttribute("post") Post post,
                                 HttpServletRequest request) {
        if (StringUtils.equals(StringUtils.trim(comment.getCommentAuthor()), "")) {
            return new Result(ResultCode.FAIL.getCode(), "请正确输入昵称！");
        }
        if (StringUtils.equals(StringUtils.trim(comment.getCommentContent()), "")) {
            return new Result(ResultCode.FAIL.getCode(), "请正确输入评论内容！");
        }
        try{
            ...
            return new Result(ResultCode.SUCCESS.getCode(),"你的评论已经提交，待博主审核之后可显示。");
        }catch (Exception e){
            return new Result(ResultCode.FAIL.getCode(),"评论失败！");
        }
    }
