...
public class AdminController extends BaseController {
    ...
    @ResponseBody
    public Result getLogin(@ModelAttribute("loginName") String loginName,
        ...
        if (StringUtils.equals(aUser.getLoginEnable(), "false") && (between < 10)) {
            return new Result(ResultCode.FAIL.getCode(), "已禁止登录，请10分钟后再试");
        }
        //验证用户名和密码
        User user = null;
        ...
        if (ObjectUtil.equal(aUser, user)) {
            ...
            return new Result(ResultCode.FAIL.getCode(), "登录失败，你还有" + (5 - errorCount) + "次机会。");
        }
    }
